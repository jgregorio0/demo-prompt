...
- The Controller test class
  `com.preving.accionformativa.web.ExtranetMultiActionController`.
- The Method to test 
  `com.preving.accionformativa.web.ExtranetMultiActionController.generarCertificadoPresencial`.
- The build file `build.gradle`.
- The acceptance criteria definition file `doc/extranet_acceptance_criteria.md`.

## Constraints

- **Framework & Libraries**: JUnit 4 (use `@Before`, `@Test`), Mockito (use `mock()`, `when()`, `verify()`), and Spring
  Test.
- **Method to test**: `handleRequestInternal(request, response)`.
- **Mocks**: Mock `MockHttpServletRequest` and `MockHttpServletResponse`. Mock `GenerarCertificadoService` manually in
  `setUp` (do not use `@Mock` or `@InjectMocks` annotations).
- **POJOs / Domain Models**: Data carrier classes, Plain Old Java Objects (POJOs), and domain models must be
...
2. **Test Implementation**
    ...
    - Test class must be
      `accion-formativa-wm/test/com.preving.accionformativa.web.ExtranetMultiActionControllerTest`.

4. **Implementation review**:
    - Self-review the generated code.
    - Run test
      `./gradlew test --tests "com.preving.accionformativa.web.formacionpresencial.VerActaControllerTest"`
